...
export const MemoizedListItem = memo<MemoizedListItemProps>(({
  ...
  return (
    ...
    >
      {badge && (
        <span className={`absolute top-2 right-2 text-white text-xs font-semibold px-2 py-1 rounded-full ${badge.className || 'bg-[var(--color-primary)]'}`}>
          {badge.text}
        </span>
      )}
          ...
          <div className="mt-2 space-y-1">
            {birthday && (
              <p className="text-sm text-[var(--color-text-secondary)]">
                誕生日: {format(new Date(birthday), "yyyy年M月d日", { locale: ja })}
              </p>
            )}
            {champagneCallSong && (
              <p className="text-sm text-[var(--color-text-secondary)]">
