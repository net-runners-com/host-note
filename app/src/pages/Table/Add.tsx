...
export default function TableAddPage() {
  ...
  const himeIdFromUrl = searchParams.get('himeId') ? parseInt(searchParams.get('himeId')!) : null;
  ...
  const [formData, setFormData] = useState<TableFormData>({
    ...
    himeIds: himeIdFromUrl ? [himeIdFromUrl] : [],
    mainCastId: 0,
    helpCastIds: [],
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logError(error, { component: 'TableAddPage', action: 'handleSubmit' });
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Card>
        <h1 className="text-2xl font-bold mb-6">卓記録を追加</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <MultiSelect
              ...
              onChange={(values) => setFormData({ ...formData, himeIds: values })}
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              メインキャスト <span className="text-[var(--color-error)]">*</span>
            </label>
            <select
              value={formData.mainCastId}
              onChange={(e) => setFormData({ ...formData, mainCastId: parseInt(e.target.value) })}
              className="w-full px-4 py-2 bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
              required
            >
          ...
          <div>
            <label className="block text-sm font-medium mb-2">ヘルプキャスト</label>
            <MultiSelect
              ...
              onChange={(values) => setFormData({ ...formData, helpCastIds: values })}
